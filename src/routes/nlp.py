...
async def index_project(request: Request, project_id: str, push_request: PushRequest):
    ...
    while has_records:
        page_chunks = await chunk_model.get_project_chunks(
            project_id=project.id, page_no=page_no)  # type: ignore
        if len(page_chunks):
            page_no += 1
        ...
        is_inserted = nlp_controller.index_into_vectordb(
            project=project, chunks=page_chunks, do_reset=push_request.do_reset, chunks_ids=chunks_ids)  # type: ignore
        if not is_inserted:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
...
@nlp_router.post("/index/amswer/{project_id}")
